...
public class SubscriberHealthIndicator implements ReactiveHealthIndicator {
    ...
    private void registerGauge() {
        Gauge.builder(METRIC_NAME, CLUSTER_UP, AtomicInteger::get).tag("type", "cluster").register(meterRegistry);
    }
    ...
    private Mono<Health> publishing() {
        return transactionGenerator.scenarios().map(Scenario::getRate).reduce(0.0, (c, n) -> c + n).filter(sum -> sum <= 0).flatMap(n -> getHealthForZeroRate());
    }
    ...
    private Mono<Health> subscribing() {
        return mirrorSubscriber.getSubscriptions().map(Scenario::getRate).reduce(0.0, (cur, next) -> cur + next).filter(sum -> sum > 0).flatMap(n -> UP).switchIfEmpty(getHealthForZeroRate());
    }

    private Mono<Health> restNetworkStakeHealth() {
        return restApiClient.getNetworkStakeStatusCode().flatMap(statusCode -> {

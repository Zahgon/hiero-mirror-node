...
class GrpcSubscriber implements MirrorSubscriber {
    ...
    private Flux<SubscribeResponse> clientSubscribe(GrpcSubscription subscription) {
        ...
        return Flux.defer(() -> grpcClient.subscribe(subscription)).retryWhen(Retry.backoff(retry.getMaxAttempts(), retry.getMinBackoff()).maxBackoff(retry.getMaxBackoff()).filter(this::shouldRetry).doBeforeRetry(r -> log.warn("Retry attempt #{} after failure: {}", r.totalRetries() + 1, StringUtils.substringBefore(r.failure().getMessage(), "\n")))).doOnError(t -> log.error("Error subscribing {}: ", subscription, t)).doOnSubscribe(s -> log.info("Starting subscriber {}: {}", subscription, subscriberProperties));
    }

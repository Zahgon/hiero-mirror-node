...
public class ExpressionConverterImpl implements ExpressionConverter {
    ...
    private static final Pattern EXPRESSION_PATTERN = Pattern.compile("\\$\\{(account|nft|token|topic|schedule)\\.(\\w+)}");
    ...
    private synchronized String doConvert(Expression expression) {
        ...
        try {
            ...
            final var transactionSupplier = type.getTransactionType().getSupplier().get();
            ...
            Retry retrySpec = Retry.backoff(Long.MAX_VALUE, Duration.ofSeconds(1L)).maxBackoff(Duration.ofSeconds(8L)).scheduler(Schedulers.newSingle("expression")).doBeforeRetry(r -> log.warn("Retry attempt #{} after failure: {}", r.totalRetries() + 1, r.failure().getMessage()));
            return Mono.defer(() -> transactionPublisher.publish(PublishRequest.builder().receipt(true).scenario(scenario).timestamp(Instant.now()).transaction(transactionSupplier.get().setMaxAttempts(1).setTransactionMemo(scenario.getMemo())).build())).retryWhen(retrySpec).map(PublishResponse::getReceipt).map(type.getIdExtractor()::apply).doOnSuccess(id -> log.info("Created {} entity {}", type, id)).doOnError(e -> log.error("Error converting expression: {}", e)).toFuture().join();
        } catch (Exception e) {
    ...
    private enum ExpressionType {
        ...
        ACCOUNT(TransactionType.ACCOUNT_CREATE, r -> r.accountId.toString()), NFT(TransactionType.TOKEN_CREATE, r -> r.tokenId.toString()), SCHEDULE(TransactionType.SCHEDULE_CREATE, r -> r.scheduleId.toString()), TOKEN(TransactionType.TOKEN_CREATE, r -> r.tokenId.toString()), TOPIC(TransactionType.CONSENSUS_CREATE_TOPIC, r -> r.topicId.toString());

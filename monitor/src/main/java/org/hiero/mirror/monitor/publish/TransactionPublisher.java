...
public class TransactionPublisher implements AutoCloseable {
    ...
    private Mono<PublishResponse.PublishResponseBuilder> processTransactionResponse(Client client, PublishRequest request, TransactionResponse transactionResponse) {
        PublishResponse.PublishResponseBuilder builder = PublishResponse.builder().request(request).timestamp(Instant.now()).transactionId(transactionResponse.transactionId);
        if (request.isSendRecord()) {
            // TransactionId.getRecord() is inefficient doing a get receipt, a cost query, then the get record
            return execute(client, new TransactionRecordQuery().setQueryPayment(Hbar.from(1, HbarUnit.HBAR)).setTransactionId(transactionResponse.transactionId)).map(r -> builder.transactionRecord(r).receipt(r.receipt));
        } else if (request.isReceipt()) {
            return execute(client, new TransactionReceiptQuery().setTransactionId(transactionResponse.transactionId)).map(builder::receipt);
        }
    ...
    private Flux<Client> getClients() {
        return nodeSupplier.refresh().collect(Collectors.toList()).flatMapMany(nodes -> Flux.range(0, publishProperties.getClients()).flatMap(i -> Mono.defer(() -> Mono.just(toClient(nodes)))));
    }
    ...
    private Client toClient(List<NodeProperties> nodes) {
        ...
        var network = nodes.stream().collect(Collectors.toMap(NodeProperties::getEndpoint, p -> AccountId.fromString(p.getAccountId()), (a, b) -> a));
        var accountIds = network.values().stream().map(AccountId::toString).collect(Collectors.toSet());
        var nodeAddresses = nodes.stream().filter(n -> accountIds.contains(n.getAccountId())).map(NodeProperties::toNodeAddress).toList();
        var nodeAddressBook = NodeAddressBook.newBuilder().addAllNodeAddress(nodeAddresses).build().toByteString();

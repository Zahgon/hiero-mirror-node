...
final class FeeTokenStore implements ReadableTokenStore {
    ...
    private static Token toToken(final TokenID id, final org.hiero.mirror.common.domain.token.Token token, final CustomFeeRepository customFeeRepository) {
        return Token.newBuilder().tokenId(id).tokenType(token.getType() == TokenTypeEnum.NON_FUNGIBLE_UNIQUE ? TokenType.NON_FUNGIBLE_UNIQUE : TokenType.FUNGIBLE_COMMON).customFees(getCustomFees(token.getTokenId(), customFeeRepository)).build();
    }
    ...
    private static List<CustomFee> getCustomFees(final long tokenId, final CustomFeeRepository customFeeRepository) {
        return customFeeRepository.findById(tokenId).filter(customFee -> !customFee.isEmptyFee()).map(customFee -> {
            final var count = size(customFee.getFixedFees()) + size(customFee.getFractionalFees()) + size(customFee.getRoyaltyFees());

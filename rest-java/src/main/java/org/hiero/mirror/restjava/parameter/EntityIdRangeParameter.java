...
public record EntityIdRangeParameter(RangeOperator operator, Long value) implements RangeParameter<Long> {
    ...
    public static final EntityIdRangeParameter EMPTY = new EntityIdRangeParameter(RangeOperator.UNKNOWN, EntityId.EMPTY);
    ...
    private static EntityId getEntityId(String entityId) {
        List<Long> parts = Splitter.on('.').splitToStream(Objects.requireNonNullElse(entityId, "")).map(Long::valueOf).filter(n -> n >= 0).toList();
        ...
        return switch(parts.size()) {
            case 1 ->
                EntityId.of(properties.getShard(), properties.getRealm(), parts.get(0));
            case 2 ->
                EntityId.of(properties.getShard(), parts.get(0), parts.get(1));
            case 3 ->
                EntityId.of(parts.get(0), parts.get(1), parts.get(2));
            default ->
                throw new IllegalArgumentException("Invalid entity ID");
        };
    }

...
public class EntityIdServiceImpl implements EntityIdService {
    ...
    private <T extends GeneratedMessage> Optional<EntityId> doLookups(T[] entityIdProtos, Function<T, Optional<EntityId>> loader) {
    ...
    private Optional<EntityId> findByEvmAddress(byte[] evmAddress, boolean throwRecoverableError) {
        var id = Optional.ofNullable(DomainUtils.fromEvmAddress(evmAddress)).or(() -> entityRepository.findByEvmAddress(evmAddress).map(EntityId::of));
    ...
    private Optional<EntityId> findByAliasEvmAddress(byte[] alias) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Trying to find entity by evm address {} recovered from public key alias {}", Hex.encodeHexString(evmAddress), Hex.encodeHexString(alias));
        }

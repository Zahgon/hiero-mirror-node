...
public class BackfillTransactionHashMigration extends RepeatableMigration {
    ...
    private static final Map<Boolean, MigrationVersion> MINIMUM_VERSION = Map.of(// false for v1, and true for v2
    Boolean.FALSE, MigrationVersion.fromVersion("1.99.1"), Boolean.TRUE, MigrationVersion.fromVersion("2.4.1"));
    ...
    public BackfillTransactionHashMigration(EntityProperties entityProperties, Environment environment, @Owner ObjectProvider<JdbcOperations> jdbcOperationsProvider, ImporterProperties importerProperties, ObjectProvider<TimePartitionService> timePartitionServiceProvider) {
    ...
    private int backfillFromTable(MigrationContext context, String sqlTemplate, String tableName) {
        ...
        var params = new MapSqlParameterSource().addValue(END_TIMESTAMP_KEY, context.endTimestamp).addValue(START_TIMESTAMP_KEY, context.startTimestamp);
        ...
        int count = 0;
        partitions = partitions.stream().filter(p -> {
    ...
    private MigrationContext getMigrationContext(long startTimestamp) {
        ...
        long endTimestamp = Objects.requireNonNull(getJdbcOperations().queryForObject(GET_END_CONSENSUS_TIMESTAMP_SQL, Long.class));
        ...
        String transactionTypesCondition = transactionHashTypes.isEmpty() ? StringUtils.EMPTY : String.format("and type in (%s)", transactionHashTypes.stream().map(TransactionType::getProtoId).map(Object::toString).collect(joining(",")));
        String backfillEthereumTransactionHashSql = ethereumTransactionIncluded ? String.format(BACKFILL_ETHEREUM_TRANSACTION_HASH_SQL, destinationTableName) : StringUtils.EMPTY;
        String backfillTransactionHashSql = String.format(BACKFILL_TRANSACTION_HASH_SQL, destinationTableName, transactionTypesCondition);
        var backfillBoth = new MigrationContext(backfillEthereumTransactionHashSql, backfillTransactionHashSql, endTimestamp, startTimestamp, v2, true);
        var backfillEthereumTransactionHash = new MigrationContext(backfillEthereumTransactionHashSql, StringUtils.EMPTY, endTimestamp, startTimestamp, v2, false);
        var backfillTransactionHash = new MigrationContext(StringUtils.EMPTY, backfillTransactionHashSql, endTimestamp, startTimestamp, v2, true);
        var strategy = getStrategy();
        return switch(strategy) {
            case AUTO ->
                tableHasData(TRANSACTION_HASH_TABLE_NAME) ? backfillEthereumTransactionHash : backfillBoth;
            case BOTH ->
                backfillBoth;
            case ETHEREUM_HASH ->
                backfillEthereumTransactionHash;
            case TRANSACTION_HASH ->
                backfillTransactionHash;
        };
    }

    private Strategy getStrategy() {
        String name = migrationProperties.getParams().getOrDefault(STRATEGY_KEY, Strategy.AUTO.name()).toUpperCase();
    ...
    private record MigrationContext(String backfillEthereumTransactionHashSql, String backfillTransactionHashSql, long endTimestamp, long startTimestamp, boolean tempTable, boolean truncate) {
    ...
    private enum Strategy {
        ...
        AUTO, BOTH, ETHEREUM_HASH, TRANSACTION_HASH
    }
}

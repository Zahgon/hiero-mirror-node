...
public class MetricsExecutionInterceptor implements ExecutionInterceptor {
    ...
    static final String METRIC_DOWNLOAD_REQUEST = "hiero.mirror.importer.stream.request";

    static final String METRIC_DOWNLOAD_RESPONSE = "hiero.mirror.importer.stream.response";
    ...
    private static final Pattern ENTITY_ID_PATTERN = Pattern.compile("/(balance|record)(s_)?(\\d{1,10})\\.\\d{1,10}\\.(\\d{1,10})");
    ...
    private final Timer.Builder requestMetric = Timer.builder(METRIC_DOWNLOAD_REQUEST).description("The time in seconds it took to receive the response from S3");

    private final DistributionSummary.Builder responseSizeMetric = DistributionSummary.builder(METRIC_DOWNLOAD_RESPONSE).description("The size of the response in bytes returned from S3").baseUnit("bytes");

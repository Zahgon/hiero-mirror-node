...
final class ContractLogIndexMigration extends AsyncJavaMigration<Long> {
    ...
    protected ContractLogIndexMigration(final Environment environment, final DBProperties dbProperties, final ImporterProperties importerProperties, @Owner final ObjectProvider<JdbcOperations> jdbcOperationsProvider, final EntityProperties entityProperties) {
    ...
    private String getVersionedContractUpdateQuery() {
        return String.format(UPDATE_CONTRACT_LOG_INDEXES, v2 ? V2_PROPERTY_MAX_INTERMEDIATE_RESULTS : StringUtils.EMPTY);
    }

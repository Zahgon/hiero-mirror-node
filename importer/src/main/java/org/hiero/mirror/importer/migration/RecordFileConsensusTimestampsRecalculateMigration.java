...
final class RecordFileConsensusTimestampsRecalculateMigration extends AsyncJavaMigration<Long> {
    ...
    private static final Map<String, Long> DEFAULT_MIN_CONSENSUS_END_TIMESTAMP_BY_NETWORK = Map.of(HederaNetwork.MAINNET, MAINNET_MIN_CONSENSUS_END_TIMESTAMP, HederaNetwork.TESTNET, TESTNET_MIN_CONSENSUS_END_TIMESTAMP);
    ...
    private static final DataClassRowMapper<RecordFileSlice> RECORD_FILE_SLICE_ROW_MAPPER = new DataClassRowMapper<>(RecordFileSlice.class);

    private static final DataClassRowMapper<GapTransactionsResult> GAP_TRANSACTIONS_RESULT_ROW_MAPPER = new DataClassRowMapper<>(GapTransactionsResult.class);
    ...
    protected RecordFileConsensusTimestampsRecalculateMigration(final Environment environment, final DBProperties dbProperties, final ImporterProperties importerProperties, @Owner final ObjectProvider<JdbcOperations> jdbcOperationsProvider) {
    ...
    private void updateSidecarFiles(RecordFileSlice block, Long consensusEndCalculated) {
        ...
        var sidecarUpdateParams = new MapSqlParameterSource().addValue("consensusEnd", block.consensusEnd()).addValue("consensusEndCalculated", consensusEndCalculated);
        int updatedSidecars = jdbc.update(UPDATE_SIDECAR_FILES_WITH_CALCULATED_TIMESTAMPS, sidecarUpdateParams);
        log.info("Updated {} sidecar_file row(s) for consensus_end {} with calculated timestamp {}.", updatedSidecars, block.consensusEnd(), consensusEndCalculated);
    }

    @Nullable
    private Long lookupPrevConsensusEnd(long consensusStart) {
        return queryForObjectOrNull(SELECT_PREV_CONSENSUS_END, new MapSqlParameterSource("consensusStart", consensusStart), Long.class);
    }

    @Nullable
    private Long lookupNextConsensusStart(long consensusEnd) {
        return queryForObjectOrNull(SELECT_NEXT_CONSENSUS_START, new MapSqlParameterSource("consensusEnd", consensusEnd), Long.class);
    }
    ...
    private GapTransactionsResult computeStartTimestamp(RecordFileSlice block) {
        ...
        var params = new MapSqlParameterSource().addValue("consensusStart", block.consensusStart()).addValue("prevConsensusEnd", prevConsensusEnd != null ? prevConsensusEnd : 0L).addValue("missingTransactionsCount", block.count() - block.actualTransactionsCount());
    ...
    private GapTransactionsResult computeEndTimestamp(RecordFileSlice block) {
        ...
        var params = new MapSqlParameterSource().addValue("consensusEnd", block.consensusEnd()).addValue("nextConsensusStart", nextConsensusStart != null ? nextConsensusStart : Long.MAX_VALUE).addValue("missingTransactionsCount", block.count() - block.actualTransactionsCount());
    ...
    private void logFailedRecalculationIfNeeded(RecordFileSlice block, long transactionsCountBeforeStart, long transactionsCountAfterEnd) {
        if (transactionsCountBeforeStart + transactionsCountAfterEnd != block.count() - block.actualTransactionsCount()) {
            log.warn("The transactions count before record_file start {} and after end {} does not match the missing" + "transactions count {} for record_file with consensus_end {}.", transactionsCountBeforeStart, transactionsCountAfterEnd, (block.count() - block.actualTransactionsCount()), block.consensusEnd());
        }
    }

    @Nullable
    private Long getTimestampOrNull(GapTransactionsResult gapTransactionsResult) {

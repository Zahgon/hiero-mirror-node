...
final class ContractNullKeyMigration extends ConfigurableJavaMigration {
    ...
    public ContractNullKeyMigration(Environment environment, ObjectProvider<NamedParameterJdbcOperations> jdbcOperationsProvider, ImporterProperties importerProperties) {
    ...
    private void update(boolean history) {
        ...
        var query = String.format("select id from entity%s where key is null and type = 'CONTRACT' and created_timestamp is not null", suffix);
        ...
        jdbcOperations.query(query, rs -> {
            ...
            byte[] key = Key.newBuilder().setContractID(id.toContractID()).build().toByteArray();

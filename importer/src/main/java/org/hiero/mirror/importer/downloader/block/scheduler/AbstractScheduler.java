...
abstract class AbstractScheduler implements Scheduler {
    ...
    private final AtomicReference<List<BlockNodeProperties>> discovered = new AtomicReference<>(Collections.emptyList());
    ...
    protected AbstractScheduler(final BlockNodeDiscoveryService blockNodeDiscoveryService, final ManagedChannelBuilderProvider channelBuilderProvider, final MeterRegistry meterRegistry, final StreamProperties streamProperties) {
    ...
    private void refreshNodes() {
        ...
        for (final var blockNodeProperties : next) {
            nodes.add(new BlockNode(channelBuilderProvider, this::drainGrpcBuffer, meterRegistry, blockNodeProperties, streamProperties));
        }
        setNodes(nodes);
    }

    @Nullable
    private static ScheduledBlockNode hasBlock(final long nextBlockNumber, final BlockNode node) {
    ...
    private static boolean nodesChanged(final List<BlockNodeProperties> current, final List<BlockNodeProperties> next) {
        ...
        for (int i = 0; i < current.size(); i++) {
            if (!Objects.equals(current.get(i), next.get(i))) {
                // A host, port, or TLS setting changed
                return true;
            }
        }
        return false;

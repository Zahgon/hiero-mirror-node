...
public class AccountEvmAddressMigration extends RepeatableMigration {
    ...
    public AccountEvmAddressMigration(ObjectProvider<NamedParameterJdbcOperations> jdbcOperationsProvider, ImporterProperties importerProperties) {
    ...
    private void updateAlias(boolean history) {
        ...
        var query = String.format("select id, alias from entity%s where evm_address is null and length(alias) = 35", suffix);

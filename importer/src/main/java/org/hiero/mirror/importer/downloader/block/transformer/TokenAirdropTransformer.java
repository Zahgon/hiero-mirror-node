...
final class TokenAirdropTransformer extends AbstractBlockTransactionTransformer {
     ...
     */
    private List<PendingAirdrop> getPendingAirdrops(TokenAirdropTransactionBody tokenAirdrop, TransactionResult transactionResult) {
        ...
        for (var tokenTransfer : tokenAirdrop.getTokenTransfersList()) {
            ...
            for (var aa : creditAccountAmounts) {
                var pendingAirdropId = PendingAirdropId.newBuilder().setReceiverId(aa.getAccountID()).setSenderId(sender).setFungibleTokenType(tokenId).build();
            ...
            for (var nftTransfer : tokenTransfer.getNftTransfersList()) {
                ...
                var pendingAirdropId = PendingAirdropId.newBuilder().setSenderId(nftTransfer.getSenderAccountID()).setReceiverId(nftTransfer.getReceiverAccountID()).setNonFungibleToken(NftID.newBuilder().setSerialNumber(nftTransfer.getSerialNumber()).setTokenID(tokenId)).build();

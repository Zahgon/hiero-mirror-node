...
public class RecordFileReaderImplV5 implements RecordFileReader {
    ...
    private void readBody(ValidatedDataInputStream vdis, DigestInputStream metadataDigestInputStream, RecordFile recordFile) throws IOException {
        ...
        while (!isHashObject(vdis, hashObjectClassId)) {
            ...
            var recordItem = RecordItem.builder().hapiVersion(recordFile.getHapiVersion()).previous(lastRecordItem).transactionRecord(TransactionRecord.parseFrom(recordStreamObject.recordBytes)).transactionIndex(count).transaction(Transaction.parseFrom(recordStreamObject.transactionBytes)).build();

...
class ConsensusCreateTopicTransactionHandler extends AbstractEntityCrudTransactionHandler {
    ...
    ConsensusCreateTopicTransactionHandler(EntityIdService entityIdService, EntityListener entityListener, ConsensusUpdateTopicTransactionHandler consensusUpdateTopicTransactionHandler) {
    ...
    private void createTopic(long consensusTimestamp, long topicId, ConsensusCreateTopicTransactionBody transactionBody) {
        ...
        var feeExemptKeyList = FeeExemptKeyList.newBuilder().addAllKeys(transactionBody.getFeeExemptKeyListList()).build().toByteArray();
        ...
        var topic = Topic.builder().adminKey(adminKey).createdTimestamp(consensusTimestamp).id(topicId).feeExemptKeyList(feeExemptKeyList).feeScheduleKey(feeScheduleKey).submitKey(submitKey).timestampRange(Range.atLeast(consensusTimestamp)).build();

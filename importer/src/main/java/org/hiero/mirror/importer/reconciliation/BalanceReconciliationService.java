...
class BalanceReconciliationService {
    ...
    static final String METRIC = "hiero.mirror.importer.reconciliation";

    // Due to the number of rows returned, it's considerably more performant to not use JPA
    private static final String BALANCE_QUERY = "select account_id, balance from account_balance " + "where consensus_timestamp = ?";
    ...
    private static final String TOKEN_BALANCE_QUERY = "select account_id, token_id, balance from token_balance " + "where consensus_timestamp = ?";
    ...
    BalanceReconciliationService(AccountBalanceFileRepository accountBalanceFileRepository, JdbcOperations jdbcOperations, MeterRegistry meterRegistry, RecordFileRepository recordFileRepository, ReconciliationProperties reconciliationProperties, ReconciliationJobRepository reconciliationJobRepository) {
    ...
    @Scheduled(cron = "${hiero.mirror.importer.reconciliation.cron:0 0 0 * * *}")
    ...
    private ReconciliationJob getLatestJob() {
        ...
        long consensusTimestamp = reconciliationJobRepository.findLatest().map(ReconciliationJob::getConsensusTimestamp).orElse(startDate);
        consensusTimestamp = Math.max(startDate, consensusTimestamp);
        var reconciliationJob = ReconciliationJob.builder().consensusTimestamp(consensusTimestamp).count(0).error("").status(RUNNING).timestampStart(Instant.now()).build();
    ...
    private void reconcile(BalanceSnapshot previous, BalanceSnapshot current) {
        ...
        log.info("Reconciled balance file {} with {} balances and {} token balances in {} ms", name, current.getBalances().size(), current.getTokenBalances().size(), elapsed);
    ...
    private <K> void reconcileTransfers(ReconciliationStatus failureStatus, Function<BalanceSnapshot, Map<K, Long>> mapper, BalanceSnapshot previous, BalanceSnapshot current) {
    ...
    private Optional<BalanceSnapshot> getNextBalanceSnapshot(ReconciliationJob reconciliationJob, Optional<BalanceSnapshot> previous) {
        long startTime = System.currentTimeMillis();
        long toTimestamp = DomainUtils.convertToNanosMax(reconciliationProperties.getEndDate());
        long fromTimestamp = previous.map(BalanceSnapshot::getAccountBalanceFile).map(AccountBalanceFile::getConsensusTimestamp).map(t -> t + 1L).orElseGet(reconciliationJob::getConsensusTimestamp);
        return accountBalanceFileRepository.findNextInRange(fromTimestamp, toTimestamp).map(accountBalanceFile -> {
            ...
            return new BalanceSnapshot(accountBalanceFile, balances, recordFile, reconciliationJob, startTime, tokenBalances);
        });
    }

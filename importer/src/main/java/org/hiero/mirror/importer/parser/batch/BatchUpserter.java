...
public class BatchUpserter extends BatchInserter {
    ...
    public BatchUpserter(Class<?> entityClass, DataSource dataSource, MeterRegistry meterRegistry, CommonParserProperties properties, UpsertQueryGenerator upsertQueryGenerator) {
        ...
        upsertMetric = Timer.builder(LATENCY_METRIC).description("The time it took to batch insert rows").tag("table", finalTableName).tag("upsert", "true").register(meterRegistry);
    }

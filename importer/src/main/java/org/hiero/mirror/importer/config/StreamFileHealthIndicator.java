...
public class StreamFileHealthIndicator implements HealthIndicator {
    ...
    private static final Health missingParseDurationTimerHealth = getHealthWithReason(Status.UNKNOWN, STREAM_PARSE_DURATION_METRIC_NAME + MISSING_TIMER_REASON);

    private static final Health missingStreamCloseTimerHealth = getHealthWithReason(Status.UNKNOWN, STREAM_CLOSE_LATENCY_METRIC_NAME + MISSING_TIMER_REASON);

    private static final Health endDateInPastHealth = getHealthWithReason(Status.UP, "EndDate has passed, stream files are no longer expected");

    private final AtomicReference<Health> lastHealthStatus = new AtomicReference<>(Health.unknown().withDetail(COUNT_KEY, 0L).withDetail(LAST_CHECK_KEY, Instant.now()).withDetail(REASON_KEY, "Starting up, no files parsed yet").// unknown until at least 1 stream file is parsed
    build());
    ...
    private Instant getStartTime() {
        return importerProperties.getStartDate() == null ? DateRangeCalculator.STARTUP_TIME : importerProperties.getStartDate();
    }

    private Health getResolvedHealthWhenNoStreamFilesParsed(Instant currentInstant, Instant lastCheck) {
        Timer streamCloseLatencyDurationTimer = meterRegistry.find(STREAM_CLOSE_LATENCY_METRIC_NAME).tags(Tags.of("type", parserProperty.getStreamType().toString())).timer();
        ...
        if (mean == 0 && currentInstant.isBefore(getStartTime().plus(parserProperty.getStreamType().getFileCloseInterval()))) {
    ...
    private static Health getHealthWithReason(Status status, String reason) {
        return Health.status(status).withDetail(REASON_KEY, reason).build();
    }

    private Health getHealthDown(Timer streamFileParseDurationTimer, Instant currentInstant, Instant lastCheck) {
        Health.Builder healthBuilder = Health.down().withDetail(REASON_KEY, String.format("No new stream stream files have been parsed since: %s", lastCheck));
    ...
    private Health getHealth(Health.Builder healthBuilder, Timer streamFileParseDurationTimer, Instant currentInstant) {
        return healthBuilder.withDetail(COUNT_KEY, streamFileParseDurationTimer.count()).withDetail(LAST_CHECK_KEY, currentInstant).withDetail("max", streamFileParseDurationTimer.max(TimeUnit.MILLISECONDS)).withDetail("mean", streamFileParseDurationTimer.mean(TimeUnit.MILLISECONDS)).withDetail("type", parserProperty.getStreamType().toString()).build();
    }
}

...
public class TransactionHashBatchInserter implements BatchPersister {
    ...
    public TransactionHashBatchInserter(DataSource dataSource, MeterRegistry meterRegistry, CommonParserProperties commonParserProperties, TransactionHashTxManager transactionHashTxManager) {
        ...
        this.shardBatchInserters = IntStream.range(0, V1_SHARD_COUNT).boxed().collect(ImmutableMap.toImmutableMap(Function.identity(), shard -> new BatchInserter(TransactionHash.class, dataSource, meterRegistry, commonParserProperties, String.format("%s_%02d", tableName, shard))));
    }
    ...
    private Mono<Void> processShard(Map.Entry<Integer, List<TransactionHash>> shardData) {
        return Mono.just(shardData).doOnNext(this::persist).subscribeOn(scheduler).then();
    }

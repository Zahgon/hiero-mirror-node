...
class FixAirdropTokenAssociationMigration extends ConfigurableJavaMigration {
    ...
    private static final RowMapper<ClaimedAirdrop> CLAIMED_AIRDROP_ROW_MAPPER = new DataClassRowMapper<>(ClaimedAirdrop.class);

    private static final RowMapper<NftTransfer> NFT_TRANSFER_ROW_MAPPER = new DataClassRowMapper<>(NftTransfer.class);

    private static final RowMapper<TokenBalanceChange> TOKEN_BALANCE_CHANGE_ROW_MAPPER = new DataClassRowMapper<>(TokenBalanceChange.class);
    ...
    FixAirdropTokenAssociationMigration(Environment environment, ImporterProperties importerProperties, ObjectProvider<NamedParameterJdbcOperations> jdbcOperationsProvider, ObjectProvider<TimePartitionService> timePartitionServiceProvider, ObjectProvider<TransactionTemplate> transactionTemplateProvider) {
    ...
    private boolean isCurrentSnapshotFull(long previousTimestamp, long currentTimestamp) {
        return timePartitionServiceProvider.getObject().getOverlappingTimePartitions("account_balance", previousTimestamp, currentTimestamp).size() > 1;
    }
     ...
     */
    private List<TokenAccountMeta> getActiveTokenAccounts(List<TokenAccountMeta> tokenAccounts, long fromTimestamp, long toTimestamp) {
        return tokenAccounts.stream().filter(tokenAccountMeta -> {
    ...
    private List<ClaimedAirdrop> getClaimedAirdrops() {
        return jdbcOperationsProvider.getObject().getJdbcOperations().queryForStream(GET_CLAIMED_AIRDROPS_SQL, CLAIMED_AIRDROP_ROW_MAPPER).filter(claimedAirdrop -> {
            var params = new MapSqlParameterSource().addValue(ACCOUNT_ID, claimedAirdrop.getAccountId()).addValue(TOKEN_ID, claimedAirdrop.getTokenId()).addValue(TIMESTAMP, claimedAirdrop.getConsensusTimestamp());
            return BooleanUtils.isTrue(jdbcOperationsProvider.getObject().queryForObject(IS_TOKEN_ACCOUNT_MISSING_SQL, params, Boolean.class));
        }).toList();
    }

    private TokenBalanceChange getFungibleTokenBalanceChange(TokenAccountMeta tokenAccountMeta, long fromTimestamp, long toTimestamp) {
        ...
        var params = new MapSqlParameterSource().addValue(ACCOUNT_ID, tokenAccount.getAccountId()).addValue(TOKEN_ID, tokenAccount.getTokenId()).addValue(FROM, fromTimestamp).addValue(TO, toTimestamp);
        try {
            return Objects.requireNonNull(jdbcOperationsProvider.getObject().queryForObject(GET_FUNGIBLE_TOKEN_BALANCE_CHANGE_SQL, params, TOKEN_BALANCE_CHANGE_ROW_MAPPER));
    ...
    private List<TokenAccountMeta> getMissingTokenAccounts(List<ClaimedAirdrop> claimedAirdrops) {
        ...
        for (var claimedAirdrop : claimedAirdrops) {
            ...
            if (Optional.ofNullable(tokenAccountState.get(id)).map(TokenAccountMeta::isAlwaysValid).orElse(false)) {
                continue;
            }
            ...
            try {
                var params = new MapSqlParameterSource().addValue(ACCOUNT_ID, id.getAccountId()).addValue(TOKEN_ID, id.getTokenId()).addValue(TIMESTAMP, createdTimestamp);
                validToTimestamp = Objects.requireNonNull(jdbcOperationsProvider.getObject().queryForObject(GET_TOKEN_ACCOUNT_VALID_TO_TIMESTAMP_SQL, params, Long.class));
            } catch (IncorrectResultSizeDataAccessException ex) {
                // ignore
            }
            var tokenAccountMeta = TokenAccountMeta.builder().fungible(claimedAirdrop.isFungible()).tokenAccount(tokenAccount).validToTimestamp(validToTimestamp).build();
    ...
    private TokenBalanceChange getNftBalanceChange(List<NftTransfer> nftTransfers, TokenAccountMeta tokenAccountMeta) {
        ...
        return lastTimestamp != null ? TokenBalanceChange.builder().change(change).consensusTimestamp(lastTimestamp).build() : null;
    }
    ...
    private void persistTokenAccounts(List<TokenAccountMeta> tokenAccountMetas) {
        var batchParams = tokenAccountMetas.stream().map(TokenAccountMeta::getTokenAccount).map(ta -> new MapSqlParameterSource().addValue(ACCOUNT_ID, ta.getAccountId()).addValue(ASSOCIATED, ta.getAssociated()).addValue(BALANCE, ta.getBalance()).addValue(BALANCE_TIMESTAMP, ta.getBalanceTimestamp()).addValue(CREATED_TIMESTAMP, ta.getCreatedTimestamp()).addValue(TIMESTAMP, ta.getTimestampLower()).addValue(TOKEN_ID, ta.getTokenId())).toArray(SqlParameterSource[]::new);
    ...
    private void persistTokenBalanceSnapshot(Collection<TokenBalance> tokeBalances) {
        jdbcOperationsProvider.getObject().getJdbcOperations().batchUpdate(INSERT_TOKEN_BALANCE_SQL, tokeBalances, tokeBalances.size(), (ps, tokenBalance) -> {
    ...
    private void updateTokenAccountBalance(TokenAccountMeta tokenAccountMeta, List<NftTransfer> nftTransfers, long fromTimestamp, long toTimestamp) {
        var balanceChange = tokenAccountMeta.isFungible() ? getFungibleTokenBalanceChange(tokenAccountMeta, fromTimestamp, toTimestamp) : getNftBalanceChange(nftTransfers, tokenAccountMeta);

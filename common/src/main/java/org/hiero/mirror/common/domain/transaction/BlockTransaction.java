...
public class BlockTransaction implements StreamItem {
    ...
    // for consensus submit message transaction
    ...
    public BlockTransaction(final BlockTransaction previous, final SignedTransaction signedTransaction, final byte[] signedTransactionBytes, final List<StateChanges> stateChanges, final List<TraceData> traceData, final TransactionBody transactionBody, final TransactionResult transactionResult, final Map<TransactionCase, TransactionOutput> transactionOutputs) {
        ...
        parentConsensusTimestamp = transactionResult.hasParentConsensusTimestamp() ? DomainUtils.timestampInNanosMax(transactionResult.getParentConsensusTimestamp()) : null;
        parent = parseParent();
        ...
        final var submitMessageTraceData = getTraceDataItem(traceDataMap, SUBMIT_MESSAGE_TRACE_DATA, TraceData::getSubmitMessageTraceData);
        if (submitMessageTraceData != null) {
            topicMessage.set(TopicMessage.builder().runningHash(DomainUtils.toBytes(submitMessageTraceData.getRunningHash())).sequenceNumber(submitMessageTraceData.getSequenceNumber()).build());
        }
        topicId = transactionBody.hasConsensusSubmitMessage() ? transactionBody.getConsensusSubmitMessage().getTopicID() : null;
    }
    ...
    private ByteString calculateTransactionHash() {
        ...
        final var transaction = Transaction.newBuilder().setBodyBytes(signedTransaction.getBodyBytes()).setSigMap(signedTransaction.getSigMap()).build();
    ...
    private StateChangeContext createStateChangeContext() {
        ...
        return !CollectionUtils.isEmpty(stateChanges) ? new StateChangeContext(stateChanges) : StateChangeContext.EMPTY_CONTEXT;
    }
    ...
    private BlockTransaction parseParent() {
        if (parentConsensusTimestamp != null && previous != null) {
            ...
            } else if (previous.parent != null && parentConsensusTimestamp.equals(previous.parent.parentConsensusTimestamp)) {
    ...
    private boolean parseSuccess() {
        ...
        return status == ResponseCodeEnum.FEE_SCHEDULE_FILE_PART_UPLOADED || status == ResponseCodeEnum.SUCCESS || status == ResponseCodeEnum.SUCCESS_BUT_MISSING_EXPECTED_OPERATION;
    }
    ...
    private static <T extends MessageLite> T getTraceDataItem(final Map<TraceData.DataCase, TraceData> data, final TraceData.DataCase dataCase, final Function<TraceData, T> getter) {
